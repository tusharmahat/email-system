var currPage=window.location.href;function navSwipe(){$("#check").is(":checked")?($(".menu-m ul").css("left","35px"),$(".menu-m ul").css("transition","all 0.5s")):($(".menu-m ul").css("left","-100%"),$(".menu-m ul").css("transition","all 0.5s"))}function isScreenSmall(){return window.matchMedia("(max-width: 768px)")}function activateTab(e,a){var i=document.getElementsByClassName(a);i.length>0&&(e.match("inbox")||e.match("home")?i[0].className+=" active":e.match("sent")?i[1].className+=" active":e.match("fav")?i[2].className+=" active":e.match("unread")?i[3].className+=" active":e.match("deleted")&&(i[4].className+=" active"))}function confirmDelete(e,a){return!!confirm(`Are you sure want to delete ${e} ${a}?`)}function confirmLogout(){return!!confirm("Are you sure want to logout?")}function showCompose(){$("#select-mail-ins").css("display","none"),$(".input-group input").val(""),$("#send-msg").val(""),$("#email--view").css("display","none"),$("#compose").css("display","block")}function sendConfirm(){$("#send-alert").modal()}function validate(){return!!($("#send-check-1").is(":checked")&&$("#send-check-2").is(":checked")&&$("#send-check-3").is(":checked")&&$("#send-check-4").is(":checked")&&$("#send-check-5").is(":checked"))||($("#help-yellow").css("background-color","#FFC107"),$("#btn-no").on("click",()=>{$("#help-yellow").css("background-color","#fafad2")}),!1)}function back(){if(!isScreenSmall().matches)return!!confirm("Are you sure want to cancel, and go back?")&&($("#compose").css("display","none"),$("#email--view").css("display","none"),$("#select-mail-ins").css("display","block"),!0)}function helpGenerate(e){var a,i;"none"===$("#help-appear").css("display")&&$("#help-appear").remove(),"subject"==e&&(a="Subject Line",i=`<p>
  ...
  </p>`),"message"==e&&(a="Introduction/Body",i=` <p>
  ...
  <p class="help-ex" onclick="showMore('closing')">
  ...
  </p>`),"example"==e&&(a="Example",i=`Dear April,<br><br>
      ...
      <p class="help-menu"><img class="icon--no-margin" src="../images/storage-danger.png" alt=""> >> Storage 95% or more</p>`),showHelp(a,i)}function showHelp(e,a){var i=`<div class="modal fade" id="help-appear" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true"
    style="display: none">
  ...
  </div>`;$("body").append(i),$("#help-appear").modal()}function showMore(e){var a,i;"subject"==e&&(a="Subject Line",i=`<p><b>A Formal example email of the subject line:</b><br>
  ...
  </p><a onclick="helpGenerate('subject')">Back</a>`),"to"==e&&(a="Recipient Line",i=`<p><b>Examples of email address:</b><br><i>Emily_Clarke99@gmail.com<br>James.Hall2020@gmail.com</i></p><br>
  <p><b>Email address names can include the following:</b>
      <br>
      Uppercase and lowercase letters: A-Z and a-z.<br>
      Digits: 0-9.<br>
  </p>
  `),"cc"==e&&(a="Carbon Copy",i=` <i>To: LucasBalotelli@gmail.com, <br> Cc: Ana.Gomes@icloud.com,
  ...
  </p>`),"message"==e&&(a="Introduction/Body",i=`<p><i>My name is Jordan Smith. I am the professor of Statistics for Saint Mary's University.
          This
          message is for all students enrolled in this course.</i></p><br>
  After the introduction write your message.
  `),"closing"==e&&(a="Closing",i=`<p><b>Closing sample:</b><br>
  ...
  `),"example"==e&&(a="Example",i=`Dear April,<br><br>
      ...
      Park Director, Indiana Parks<br>
      April_Knope@hotmail.com<br>
      (555) 555-6546`),$(".modal-bod").html(i)}$(document).ready(function(){$.fn.scrollDivToElement=function(e){return this.length?this.each(function(){let a=$(this),i=a.find(e);i.length>0&&a.scrollTop(a.scrollTop()-a.offset().top+i.offset().top-a.outerHeight()/2+i.outerHeight()/2)}):this};var e=currPage.split("-");isScreenSmall().matches?activateTab(currPage,"tab-m"):($("#middle").scrollDivToElement(`#view-${e[1]}`),activateTab(currPage,"tab")),$(function(){currPage.match("home")&&fetch("/users/unread-count").then(e=>e.json()).then(e=>{e.count>0&&($(".menu-m ul").css("left","35px"),confirm(`You have ${e.count} unread emails, Do you want to read them now?`)&&(window.location.href=e.url))})}),1==$("#email--view").parents("#right").length&&($("#middle-m").css("display","none"),$("#right").css("display","block")),0==$("#middle").children().length&&$("#middle").html('<p class="help--yellow">No emails</p>'),0==$("#middle-m").children().length&&$("#middle-m").html('<p class="help--yellow">No emails</p>'),$(".btn-compose").on("click",()=>{isScreenSmall().matches&&($("#middle-m").css("display","none"),$("#right").css("display","block"),$(".menu-m ul").css("left","-100%"))}),$(".btn-back").on("click",()=>{isScreenSmall().matches&&($("#right").css("display","none"),$("#middle-m").css("display","block"))})}),$(".text-search").keyup(function(){var e=$(this).val();if(""===e){$("#filter-records").html("");return}var a="";fetch(`/search/${a=currPage.includes("inbox")||currPage.includes("home")?"inbox":currPage.includes("sent")?"sent":"deleted"}`).then(e=>e.json()).then(i=>{var o=RegExp(e,"i"),s="",t=0;$.each(i,function(e,i){(-1!=i.from.search(o)||-1!=i.to.search(o)||-1!=i.sb.search(o))&&(s+=`<div class="view">${"inbox"==a?`<form class="btn-fav" action="/users/${a}/add-to-fav/${t}" method="POST">
          <button type="submit">
            <i
              class="icon${!0==i.fav?" icon-fav":""} far fa-star"
            ></i>
          </button>
        </form>`:""}
          <a href="/${a}/-${t}"
            ><label class="list-mail">
              <label
                id="view"
                name="view"
                class="${"inbox"!=a?"":"unread"}${!0!=i.read?"":"unread"}"
              >
              ${"inbox"==a?i.from:i.to} </label
              >
              <label data-role="button" class="list-sb">${i.sb}</label></label
            ></a
          >
          <form
            class="btn-delete"
            action="/users/${a}/delete-one/${t}"
            method="POST"
        ...
        </div>`),t++}),s+="</div>",$(".view").remove(),$("#middle").append(s),$("#middle-m").append(s)})});